#include <ttkCombinatorialVectorField.h>
...
vtkStandardNewMacro(ttkCombinatorialVectorField);
 ...
 */
ttkCombinatorialVectorField::ttkCombinatorialVectorField() {
 ...
 */
int ttkCombinatorialVectorField::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
 ...
 */
int ttkCombinatorialVectorField::FillOutputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
 ...
 */
int ttkCombinatorialVectorField::RequestData(vtkInformation *ttkNotUsed(request),
  ...
  //       call SetInputArrayToProcess (see Testing.xml file).
  //
  ...
  // outputArray->SetNumberOfComponents(inputArray->GetNumberOfComponents()); // only one component per tuple
  ...
  //                     (status = this->computeCriticalPoints<VTK_TT, TTK_TT>(
  ...
  vtkWarningMacro(<< "Input points: " << inputDataSet->GetNumberOfPoints());
  vtkWarningMacro(<< "Output points: " << outputDataSet->GetNumberOfPoints());
  vtkWarningMacro(<< "Input cells: " << inputDataSet->GetNumberOfCells());
  vtkWarningMacro(<< "Output cells: " << outputDataSet->GetNumberOfCells());
