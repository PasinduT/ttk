...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("CombinatorialVectorField");

  // ---------------------------------------------------------------------------
  // Initialize ttkCombinatorialVectorField module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto combinatorialVectorField = vtkSmartPointer<ttkCombinatorialVectorField>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // combinatorialVectorField->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkCombinatorialVectorField filter
      combinatorialVectorField->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    combinatorialVectorField->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkCombinatorialVectorField filter
  // ---------------------------------------------------------------------------
  combinatorialVectorField->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < combinatorialVectorField->GetNumberOfOutputPorts(); i++) {
      auto output = combinatorialVectorField->GetOutputDataObject(i);
